...
export class XStatic {
     ...
     */
    constructor(private app: XyApp, private sys: XyPrissXHSC) {
     ...
     * ```
     */
    public define(urlPath: string, diskPath: string, options: StaticOptions = {}): void {
        ...
        const root = this.sys.vars?.__root__ || process.cwd();
        const absoluteDiskPath = isAbsolute(diskPath) 
            ? diskPath 
            : normalize(join(root, diskPath));

        this.logger.info("server", `[XStatic] Serving ${urlPath} -> ${absoluteDiskPath}`);
        ...
        this.app.get(`${urlPath}/*`, async (req, res) => {
            ...
            if (!mergedOptions.allowOutsideRoot && !mergedOptions.unsafe) {
                ...
                // Block directory traversal or escape from root
                if (rel.startsWith("..") || isAbsolute(rel)) {
                    this.logger.warn("security", `[XStatic] Blocked traversal attempt: ${fullPath}`);
                    return res.status(403).send("Forbidden: Directory traversal blocked");
                }
            }

            // 3. Dotfiles check
            if (mergedOptions.dotfiles === "deny" && requestedFile.split('/').some((p: string) => p.startsWith('.'))) {
                return res.status(403).send("Forbidden: Access to dotfiles is denied");
            }

            // 4. Delegation to XHSC (Go) for high-performance streaming
            try {
                if ((res as any).xStatic) {
                    ...
                    // Fallback to standard sendFile
                    await res.sendFile(fullPath, {
                        maxAge: 86400,
                    });
                }
            } catch (err) {
                // 5. Update LRU on failure (Negative cache)
     ...
     */
    public serve(urlPath: string, diskPath: string, options: StaticOptions = {}): void {
