 ...
 */
class MetaCache {
    private cache: Map<string, { exists: boolean; expires: number }> = new Map();
    private keys: string[] = [];
...
export class XStatic {
     ...
     */
    public define(route: string, dir: string, options: StaticOptions = {}): void {
        ...
        this.app.use(normalizedRoute, async (req: Request, res: Response, next: () => void) => {
            try {
                ...
                if (req.path === normalizedRoute) {
                    ...
                    const prefix = normalizedRoute.endsWith("/") ? normalizedRoute : `${normalizedRoute}/`;
                    if (req.path.startsWith(prefix)) {
                        relativePath = req.path.substring(normalizedRoute.length);
                    }
                }
                ...
                if (worker) {
                    ...
                    res.status(500).send("[XStatic] High-performance static serving requires XHSC Clustering mode.");
                }
            } catch (err) {
        ...
        this.logger.info("server", `Static route defined: ${normalizedRoute} -> ${dir}`);
    }

    /**
     * Alias for define() - Deprecated
     */
    public serve(route: string, dir: string, options: StaticOptions = {}): void {
        this.logger.warn("server", "XStatic.serve() is deprecated. Please use XStatic.define().");
