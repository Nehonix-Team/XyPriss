...
mkdir -p "$RESULTS_DIR"
OUTPUT="$RESULTS_DIR/express.txt"

echo "==========================================" | tee "$OUTPUT"
echo " Baseline: Express + serve-static"         | tee -a "$OUTPUT"
echo "==========================================" | tee -a "$OUTPUT"

command -v node >/dev/null 2>&1 || { echo "[!] node not found"; exit 1; }
command -v autocannon >/dev/null 2>&1 || { echo "[!] autocannon not found. Run: xfpm i -g autocannon"; exit 1; }

cd "$(dirname "$0")/../baseline-express"
[ ! -d node_modules ] && xfpm install --silent

ulimit -n 65535

echo "[*] Starting Express server..." | tee -a "$OUTPUT"
node server.js &
SERVER_PID=$!
...
echo "[*] Stopping Express." | tee -a "$OUTPUT"
