...
2.  **Filtrage du Framework** : Le système parcourt la pile et ignore tous les fichiers appartenant au cœur de XyPriss (ex: `sys.ts`, `EnvApi.ts`, `ProjectDiscovery.ts`).
3.  **Localisation du Caller** : Le premier fichier trouvé qui n'appartient pas au framework est considéré comme l'origine de l'appel.
