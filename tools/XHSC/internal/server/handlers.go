...
func (s *ServerState) handleJsWorker(w http.ResponseWriter, r *http.Request, params map[string]string) {
	...
	if strings.Contains(contentType, "multipart/form-data") {
		...
		if s.FileUpload.MaxFiles > 0 && totalFiles > s.FileUpload.MaxFiles {
			log.Printf("[WARN] Upload rejected: total files %d exceeds limit %d", totalFiles, s.FileUpload.MaxFiles)
			uploadErrors = append(uploadErrors, ipc.UploadError{
				Message: fmt.Sprintf("Too many files. Maximum allowed: %d", s.FileUpload.MaxFiles),
				Type:    "LIMIT_FILE_COUNT",
			})
			goto finalize_request // Stop processing files and move to request finalization
		}
		...
		for fieldName, fileHeaders := range r.MultipartForm.File {
			...
			for _, fileHeader := range fileHeaders {
				...
				// Binary Sniffing for MIME type validation
				sniffBuf := make([]byte, 512)
				n, _ := file.Read(sniffBuf)
				detectedMime := http.DetectContentType(sniffBuf[:n])
				pureDetected, _, _ := mime.ParseMediaType(detectedMime)
				...
				if len(s.FileUpload.AllowedMimes) > 0 {
					...
					if !allowed {
						log.Printf("[WARN] File upload rejected: %s MIME type %s (detected %s) not allowed", fileHeader.Filename, fileHeader.Header.Get("Content-Type"), pureDetected)
						uploadErrors = append(uploadErrors, ipc.UploadError{
							FieldName: fieldName,
							FileName:  fileHeader.Filename,
							Message:   fmt.Sprintf("MIME type %s (detected %s) not allowed", fileHeader.Header.Get("Content-Type"), pureDetected),
				...
				jsFiles = append(jsFiles, ipc.JsFile{
					...
					Path:         finalPath,
					Size:         size,
					MimeType:     detectedMime, // Use detected MIME for more accuracy
					Encoding:     "7bit",
