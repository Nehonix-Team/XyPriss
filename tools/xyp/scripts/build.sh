#!/bin/bash
# *****************************************************************************
# Nehonix XyPriss Build Script - xfpm
...
TARGETS=(
    "x86_64-unknown-linux-gnu|linux-amd64||mimalloc"
    "x86_64-unknown-linux-musl|linux-amd64-musl||mimalloc"
    "aarch64-unknown-linux-gnu|linux-arm64||mimalloc"
    "x86_64-pc-windows-gnu|windows-amd64|.exe|mimalloc"
    "aarch64-pc-windows-gnullvm|windows-arm64|.exe|mimalloc"
...
if cargo build --release; then
    mkdir -p bin
    cp target/release/xfpm bin/xfpm
    print_success "Host binary saved to bin/xfpm"
else
...
for ENTRY in "${TARGETS[@]}"; do
    ...
    if RUSTFLAGS="-C link-arg=-s" cargo zigbuild --target "$RUST_TARGET" --release --jobs 4 $FEATURE_FLAG; then
        cp "target/$RUST_TARGET/release/xfpm${EXT}" "$DEST_PATH"
