...
/**
 * XyPriss CLI Installer (XFPM Engine)
 *
 * This script automatically downloads the appropriate XFPM (Rust) binary
...
function detectPlatform() {
    ...
    switch (platform) {
        case "darwin":
            binaryName =
                arch === "arm64" ? "xfpm-darwin-arm64" : "xfpm-darwin-amd64";
            break;
        case "linux":
            binaryName =
                arch === "arm64" ? "xfpm-linux-arm64" : "xfpm-linux-amd64";
            break;
        case "win32":
            binaryName =
                arch === "arm64"
                    ? "xfpm-windows-arm64.exe"
                    : arch === "arm"
                      ? "xfpm-windows-arm.exe"
                      : "xfpm-windows-amd64.exe";
            break;
        default:
...
async function install() {
    try {
        ...
        if (isBinaryInstalled()) {
            success(
                "XFPM is already installed and working! Run `xfpm --help` or using the alias `xyp --help` to get started.",
            );
            return;
        }
        ...
        success(`XFPM installed successfully!`);
